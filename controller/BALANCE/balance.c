...
void Balance_task(void *pvParameters)
{
	...
	while (1)
	{
		...
		if (Time_count > CONTROL_DELAY + 230)
		{
			...
			if (Allow_Recharge == 1)
			{
				...
				if (nav_walk == 1 && RED_STATE == 0)
					Drive_Motor(Recharge_UP_Move_X, Recharge_UP_Move_Z);
				// 接收到了红外信号，接收来自回充装备的回充控制命令
				else if (RED_STATE != 0)
					nav_walk = 0, Drive_Motor(Recharge_Red_Move_X, Recharge_Red_Move_Z);
				// 防止没有红外信号时小车运动
			...
			else
			{
				...
				// CAN、串口1、串口3(ROS)控制直接得到2轴目标速度，无须额外处理
					...
					Drive_Motor(Move_X, Move_Z); // CAN、串口1、串口3(ROS)控制
			}
		}
		...
		if ((Turn_Off(Voltage) == 0 && robot_mode_check_flag == 0) || (Allow_Recharge && EN && !Flag_Stop))
		{
			...
			if (Car_Mode == 0)
				Set_Pwm(MOTOR_A.Motor_Pwm, MOTOR_B.Motor_Pwm, Servo); // MD36
			else if (Car_Mode == 1)
				Set_Pwm(MOTOR_A.Motor_Pwm, MOTOR_B.Motor_Pwm, Servo); // MD36
			else if (Car_Mode == 2)
				Set_Pwm(-MOTOR_A.Motor_Pwm, -MOTOR_B.Motor_Pwm, Servo); // MD60
			else if (Car_Mode == 3)
				Set_Pwm(-MOTOR_A.Motor_Pwm, -MOTOR_B.Motor_Pwm, Servo); // MD60
			else if (Car_Mode == 4)
				Set_Pwm(-MOTOR_A.Motor_Pwm, -MOTOR_B.Motor_Pwm, Servo); // MD60
				...
				Set_Pwm(-MOTOR_A.Motor_Pwm, -MOTOR_B.Motor_Pwm, Servo); // 测试专用
			else if (Car_Mode == 8)
				Set_Pwm(-MOTOR_A.Motor_Pwm, -MOTOR_B.Motor_Pwm, Servo); // 测试专用
#elif Diff_Car
			// Set different PWM control polarity according to different car models
			// 根据不同小车型号设置不同的PWM控制极性
			if (Car_Mode == 0)
				Set_Pwm(MOTOR_A.Motor_Pwm, MOTOR_B.Motor_Pwm, 0); // MD36
			else if (Car_Mode == 1)
				Set_Pwm(MOTOR_A.Motor_Pwm, MOTOR_B.Motor_Pwm, 0); // MD36
			else if (Car_Mode == 2)
				Set_Pwm(-MOTOR_A.Motor_Pwm, -MOTOR_B.Motor_Pwm, 0); // MD60
			else if (Car_Mode == 3)
				Set_Pwm(-MOTOR_A.Motor_Pwm, -MOTOR_B.Motor_Pwm, 0); // MD60
			else if (Car_Mode == 4)
				Set_Pwm(-MOTOR_A.Motor_Pwm, -MOTOR_B.Motor_Pwm, 0); // MD60
...
int Incremental_SERVO(float SlidePosition, float SlideTarget)
{
	...
	if (Car_Mode == 2 || Car_Mode == 3)
	{
		...
		if (SlideTarget < 0 || SlidePosition < -200)
			Pwm += 0.004 * 1.6 * (Bias - Last_bias) + 0.009 * 1.6 * Bias;
		else
			Pwm += 0.004f * (Bias - Last_bias) + 0.009f * Bias;
	}
...
void Get_Velocity_Form_Encoder(void)
{
	...
	MOTOR_B.Encoder = Encoder_B_pr * CONTROL_FREQUENCY / Encoder_precision * Wheel_perimeter; // B motor 18:1 gear ratio
}
/**************************************************************************
...
#define ERROR_PWM 8000 // pwm预警值
	static u8 once = 1;
	if (once)
		check_a = 0, check_b = 0, check_c = 0, check_d = 0, once = 0;

	if (EN == 1 && robot_mode_check_flag == 0) // 保留可以使用急停开关跳过自检的功能
	{
		// 拥有一定pwm参数值后，若编码器数据不变。错误类型为：编码器未接线、驱动未接线或负责超重
		if (float_abs(MOTOR_A.Motor_Pwm) > 5500 && check_a < 500)
			robot_mode_check_flag = 1, LED_B = 0;
		if (float_abs(MOTOR_B.Motor_Pwm) > 5500 && check_b < 500)
			robot_mode_check_flag = 1, LED_B = 0;
		if (float_abs(MOTOR_C.Motor_Pwm) > 5500 && check_c < 500)
			robot_mode_check_flag = 1, LED_B = 0;
		if (float_abs(MOTOR_D.Motor_Pwm) > 5500 && check_d < 500)
			robot_mode_check_flag = 1, LED_B = 0;

		// 若存在负数，说明存在方向相反的情况：错误类型为：车型选错、驱动接线错误或编码器接线错误
		if (check_a < -3000 || check_b < -3000 || check_c < -3000 || check_d < -3000)
			robot_mode_check_flag = 1, LED_G = 0;
